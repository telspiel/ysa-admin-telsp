...
$(function () {
  $("#userId").val(Cookie.get("RESWPKEY"));
  $("#userPass").val(Cookie.get("RESWPVAL"));
  if (Cookie.get("RESWPKEY") && Cookie.get("RESWPVAL")) {
...
function proceedWithLoggedinuser(data, params) {
  ...
  if (rememberMe) {
    Cookie.set("RESWPKEY", params.username);
    Cookie.set("RESWPVAL", params.password);
  } else {
    Cookie.del("RESWPKEY");
    Cookie.del("RESWPVAL");
  }
  ...
  User.setName(userData.username)
    ...
    .setLastLogin(userData.lastLoginTime)
